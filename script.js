...
class TrueAudioDetector {
    ...
    constructor() {
        ...
        this.tiers = {
            ...
            hq:       { min: 18500, color: 'var(--color-good)',      label: 'Kualitas Tinggi', score: 85  },
            moderate: { min: 16000, color: 'var(--color-moderate)',  label: 'Sedang',           score: 60  },
            low:      { min: 14000, color: 'var(--color-bad)',       label: 'Kualitas Rendah', score: 30  },
            fake:     { min: 0,     color: 'var(--color-bad)',       label: 'Palsu/Upscaled',  score: 10  }
        };
    ...
    handleFiles(files) {
        const audio = Array.from(files).filter(f => {
            ...
            if (!ok) this.showToast(`Dilewati: ${f.name} (format tidak didukung)`, 'warning');
            return ok;
        });
        ...
        this.showToast(`${audio.length} file dalam antrian`, 'success');
    ...
    async analyzeAudio(fileData) {
        ...
        try {
            ...
            this.addStep(card, `Terdekode: ${durStr} · ${srKhz} kHz · ${chStr}`, 'done');
            ...
            this.addStep(card, `FFT ${this.settings.fftSize}-point · 64 jendela…`);
            ...
            const verdict = this.evaluateQuality(analysis, audioBuffer.sampleRate, meta.format, meta.bitrate);
            this.addStep(card, `Hasil: ${verdict.qualityLabel} — Skor ${verdict.qualityScore}`, 'done');
            ...
            this.showToast(`Gagal: ${file.name}`, 'error');
            this.processingQueue.delete(id);
    ...
    /**
     * Pick the best 12-second segment for spectral analysis.
     * Samples 3 positions (20%, 50%, 75%) and returns the one
     * with the highest high-frequency energy — crucial for lossless
     * files where a quiet musical passage could be misidentified.
     */
    bestSegment(data, sampleRate, maxSecs = 12.0) {
        const maxLen = Math.floor(sampleRate * maxSecs);
        if (data.length <= maxLen) return data;

        const fftN  = Math.min(this.settings.fftSize, 2048);
        const hann  = this.hannWindow(fftN);
        const re    = new Float32Array(fftN);
        const im    = new Float32Array(fftN);
        const nyq   = sampleRate / 2;
        const hfBin = Math.floor((9000 / nyq) * (fftN >> 1));  // 9 kHz start

        let bestSeg   = null;
        let bestScore = -Infinity;

        [0.20, 0.45, 0.70].forEach(pos => {
            const start = Math.min(
                Math.floor(data.length * pos),
                data.length - maxLen
            );
            const seg  = data.subarray(start, start + maxLen);

            // Quick HF energy scan: 6 evenly-spaced windows
            const hop  = Math.floor(seg.length / 7);
            let hfSum  = 0;
            for (let w = 0; w < 6; w++) {
                const off = w * hop;
                if (off + fftN > seg.length) break;
                for (let i = 0; i < fftN; i++) re[i] = seg[off + i] * hann[i];
                im.fill(0);
                this.fft(re, im);
                const bins = fftN >> 1;
                for (let i = hfBin; i < bins; i++) hfSum += re[i]*re[i] + im[i]*im[i];
            }
            if (hfSum > bestScore) { bestScore = hfSum; bestSeg = seg; }
        });

        return bestSeg || data.subarray(0, maxLen);
    }

    /** @deprecated Use bestSegment instead. Kept for internal compatibility. */
    midSegment(data, sampleRate, maxSecs = 12.0) {
        return this.bestSegment(data, sampleRate, maxSecs);
    }
    ...
    avgSpectrum(data, fftSize) {
        ...
        for (let s = 0; s + fftSize <= data.length && wins < 64; s += hop) {
    ...
    async spectralAnalysis(audioBuffer) {
        ...
        const seg  = this.bestSegment(mono, sampleRate, 12.0);
        const freq = this.avgSpectrum(seg, fftSize);

        /* Yield again after heavy FFT work */
        await new Promise(r => setTimeout(r, 0));
        ...
        // Store raw PCM (≤15 s) for on-demand 2D spectrogram generation
        const maxPcmLen  = Math.min(mono.length, Math.floor(sampleRate * 15));
        const pcmSegment = new Float32Array(mono.subarray(0, maxPcmLen)); // copy to avoid holding full buffer

        return { cutoff, dynRange, hfEnergy, flatness, peaks, freq, wform,
                 sampleRate, confidence, pcmSegment };
    }
    ...
    findCutoff(freq, sampleRate) {
        ...
        const threshold = refLevel - 72; // relative: 72 dB below midrange peak
        // (55 was too strict — lossless HF content can be 60-70 dB below midrange naturally)
        let   last      = 0;
    ...
    evaluateQuality(analysis, sampleRate, format = '', bitrate = 0) {
        const { cutoff, hfEnergy, flatness, dynRange, confidence } = analysis;
        const hz    = cutoff.frequency;
        const nyq   = sampleRate / 2;

        /* ── Ratio vs Nyquist — relative so any sample rate is handled ──
         *   ≥ 0.88 → Lossless     (48kHz FLAC cutoff ≥ 21.1 kHz)
         *   ≥ 0.80 → High Quality (320 kbps MP3 / 44.1 kHz ≈ 0.86)
         *   ≥ 0.68 → Moderate     (192 kbps ≈ 0.77)
         *   ≥ 0.55 → Low Quality
         *   < 0.55 → Fake / Upscaled
         *
         * NOTE: Format (FLAC/WAV) is NOT used to auto-classify as Lossless
         * because a fake FLAC (MP3 → FLAC transcode) would give a false
         * positive. Spectral content is always the primary judge.            */
        const ratio = Math.min(1.0, hz / nyq);
        let tier, score;

        if      (ratio >= 0.88) { tier = this.tiers.lossless; score = 92 + ratio * 8; }
        else if (ratio >= 0.80) { tier = this.tiers.hq;       score = 78 + ((ratio - 0.80) / 0.08) * 14; }
        else if (ratio >= 0.68) { tier = this.tiers.moderate; score = 52 + ((ratio - 0.68) / 0.12) * 26; }
        else if (ratio >= 0.55) { tier = this.tiers.low;      score = 22 + ((ratio - 0.55) / 0.13) * 30; }
        else                    { tier = this.tiers.fake;     score = Math.max(4, (ratio / 0.55) * 22); }
        ...
        // Upscale penalty
        if (cutoff.isArtificial) score -= 20;

        // Floor for clean lossless tier
        if (ratio >= 0.88 && !cutoff.isArtificial) score = Math.max(score, 91);
    ...
    /* ──────────────────────────────────────────────────────────
       8b. PROPER 2-D TIME-FREQUENCY SPECTROGRAM
    ─────────────────────────────────────────────────────────── */

    /**
     * Generate a compact 2-D spectrogram from raw PCM data.
     * Output: { grid: Float32Array[0-1], cols, bins, sampleRate }
     * grid is row-major: pixel(x,y) = grid[y * cols + x]  BUT we store
     * column-major (col × binCount) for efficient column rendering.
     */
    async generateSpectrogramData(pcmSegment, sampleRate) {
        const specFftSize = 2048;
        const binCount    = specFftSize >> 1;
        const total       = pcmSegment.length;
        const targetCols  = 900;                // display target

        // Adaptive hop: cover full segment in targetCols columns
        const rawHop  = Math.max(256, Math.floor(total / targetCols));
        const hop     = Math.min(8192, rawHop);
        const numCols = Math.min(targetCols, Math.floor((total - specFftSize) / hop) + 1);

        const hann = this.hannWindow(specFftSize);
        const re   = new Float32Array(specFftSize);
        const im   = new Float32Array(specFftSize);

        // Compact storage: normalised [0,1] in Uint8 (saves 75% vs Float32)
        const grid = new Uint8Array(numCols * binCount);
        const norm = 1.0 / (specFftSize * specFftSize);
        const dMin = -90, dRange = 90; // -90..0 dBFS

        for (let ci = 0; ci < numCols; ci++) {
            const pos = ci * hop;
            if (pos + specFftSize > total) break;

            for (let i = 0; i < specFftSize; i++) re[i] = pcmSegment[pos + i] * hann[i];
            im.fill(0);
            this.fft(re, im);

            const base = ci * binCount;
            for (let i = 0; i < binCount; i++) {
                const db = 10 * Math.log10(Math.max(1e-15, (re[i]*re[i] + im[i]*im[i]) * norm));
                grid[base + i] = Math.round(Math.max(0, Math.min(255,
                    ((db - dMin) / dRange) * 255
                )));
            }

            if (ci % 60 === 59) await new Promise(r => setTimeout(r, 0)); // yield
        }

        return { grid, cols: numCols, bins: binCount, sampleRate };
    }

    /** Render stored spectrogram data onto a canvas. */
    drawSpectrogramData(canvas, specData, cutoffBin = 0) {
        if (!canvas || !specData) return;
        const { grid, cols, bins } = specData;
        const W   = canvas.width  = canvas.offsetWidth || canvas.width;
        const H   = canvas.height;
        const ctx = canvas.getContext('2d');
        ...
        for (let xi = 0; xi < W; xi++) {
            const ci   = Math.min(cols - 1, Math.floor((xi / W) * cols));
            const base = ci * bins;

            for (let yi = 0; yi < H; yi++) {
                const freqRatio = 1 - (yi / H);         // top = high freq
                const bi  = Math.min(bins - 1, Math.floor(freqRatio * bins));
                const t   = grid[base + bi] / 255;      // [0,1]
                const pi  = (yi * W + xi) * 4;

                if (t < 0.02) {
                    pix[pi+3] = 255; continue; // black (alpha already 0 in rgba)
                }
                // Inline heat map
                let r, g, b;
                if      (t < 0.25) { const s=t*4;       r=~~(s*20);       g=0;            b=~~(s*50);     }
                else if (t < 0.50) { const s=(t-.25)*4; r=~~(20+s*200);   g=~~(s*8);      b=~~(50+s*20);  }
                else if (t < 0.75) { const s=(t-.50)*4; r=220;            g=~~(8+s*110);  b=~~(70-s*50);  }
                else               { const s=(t-.75)*4; r=255;            g=~~(118+s*137);b=~~(20+s*160); }
        ...
        // Overlay: frequency grid lines + labels
        const nyq = specData.sampleRate / 2;
        [4000, 8000, 12000, 16000, 20000].filter(f => f < nyq).forEach(hz => {
            const y = H * (1 - hz / nyq);
            ...
            ctx.textAlign = 'right';
            ctx.fillText(`${hz >= 1000 ? hz/1000+'k' : hz}`, W - 4, y - 2);
        });

        // Cutoff frequency line
        if (cutoffBin > 0) {
            const cy = H * (1 - cutoffBin / bins);
    ...
    /** Download the full-spectrogram canvas as a high-res PNG. */
    downloadSpectrogramCanvas(result) {
        const fname = result.fileName.replace(/\.[^.]+$/, '');

        const dlW = 2200, dlH = 600;
        const dlCanvas = Object.assign(document.createElement('canvas'), { width: dlW, height: dlH });

        if (result.pcmSegment && result.sampleRate) {
            // Render synchronously at download resolution using stored data
            // (reuse stored specData if available)
            const specData = result._specData;
            if (specData) {
                this.drawSpectrogramData(dlCanvas, specData, result.cutoff?.bin || 0);
            } else {
                // Fallback: draw averaged spectrum
                this.drawSpectrogram(dlCanvas, result);
            }
        } else {
            this.drawSpectrogram(dlCanvas, result);
        }

        // Burn in metadata label
        const ctx = dlCanvas.getContext('2d');
        ctx.fillStyle = 'rgba(0,0,0,0.55)';
        ctx.fillRect(0, dlH - 28, dlW, 28);
        ctx.fillStyle = '#ccc';
        ctx.font = '13px monospace';
        ctx.textAlign = 'left';
        ctx.fillText(
            `${result.fileName}  |  ${result.sampleRate}  |  ${result.bitrate} kbps  |  ${result.qualityLabel}  |  Cutoff: ${(result.cutoff?.frequency/1000).toFixed(1)} kHz`,
            10, dlH - 9
        );

        const link = Object.assign(document.createElement('a'), {
            href:     dlCanvas.toDataURL('image/png'),
            download: `spektrogram-${fname}.png`
        });
        document.body.appendChild(link);
        link.click();
        link.remove();
        this.showToast('Spektrogram diunduh!', 'success');
    }
    ...
    toggleExpand(card) {
        ...
        if (opening) {
            ...
            if (id && this.results.has(id)) {
                ...
                if (fc) {
                    fc.width = fc.parentElement?.clientWidth || 700;

                    if (result._specData) {
                        // Already generated — just draw
                        this.drawSpectrogramData(fc, result._specData, result.cutoff?.bin || 0);
                    } else if (result.pcmSegment) {
                        // Generate 2-D spectrogram progressively
                        fc.height = Math.max(180, fc.height);
                        ...
                        ctx.textAlign = 'center';
                        ctx.fillText('Membuat spektrogram…', fc.width / 2, fc.height / 2);

                        this.generateSpectrogramData(result.pcmSegment, result.sampleRateHz)
                            .then(specData => {
                                result._specData = specData; // cache
                                fc.width = fc.parentElement?.clientWidth || 700;
                                this.drawSpectrogramData(fc, specData, result.cutoff?.bin || 0);
                            });
                    } else {
                ...
                const dlBtn = card.querySelector('.dl-spec-btn');
                if (dlBtn && !dlBtn._wired) {
                    dlBtn._wired = true;
                    dlBtn.addEventListener('click', e => {
                        e.stopPropagation();
                        this.downloadSpectrogramCanvas(result);
                    });
                }
            }
        }
    ...
    fillDetailedStats(card, r) {
        ...
        if (!c) return;
        const items = [
            ['Saluran',         r.channelLabel || '—'],
            ['Spektral Flat.',  r.flatness?.toFixed(5) || '—'],
            ['Jml. Peak',       r.peaks?.length ?? 0],
            ['Upscaled?',       r.isUpscaled ? '⚠ Ya' : '✓ Tidak'],
            ...(r.peaks?.length ? [['Peak Tertinggi', `${(r.peaks[0].frequency/1000).toFixed(2)} kHz`]] : [])
        ];
    ...
    attachAudioPlayer(card, file) {
        ...
        playBtn.addEventListener('click', e => {
            ...
            if (audio.paused) {
                ...
                audio.play().catch(() => this.showToast('Pemutaran gagal', 'error'));
    ...
    updateBatchControls() {
        ...
        if (cnt)  cnt.textContent    = `${total} file dalam antrian`;
    }
    ...
    togglePause() {
        ...
        if (this.isPaused) {
            ...
            this.showToast('Pemrosesan dijeda', 'info');
        } else {
            btn.innerHTML = '<i class="fa-solid fa-pause"></i> Jeda';
            this.showToast('Pemrosesan dilanjutkan', 'success');
            this.processQueue();
        }
    }
    ...
    cancelQueue() {
        ...
        this.queue.clear();
        this.updateBatchControls();
        this.showToast(`${n} file pending dibatalkan`, 'warning');
    }
    ...
    exportResults(fmt = 'csv') {
        if (!this.results.size) { this.showToast('Tidak ada hasil untuk diekspor', 'warning'); return; }
        ...
        this.showToast(`Diekspor sebagai ${fmt.toUpperCase()}`, 'success');
    }
    ...
    cycleSort() {
        const modes = [
            { label: 'Kualitas ↓',  fn: (a,b) => b.qualityScore  - a.qualityScore  },
            { label: 'Kualitas ↑',  fn: (a,b) => a.qualityScore  - b.qualityScore  },
            { label: 'Nama A→Z',    fn: (a,b) => a.fileName.localeCompare(b.fileName) },
            { label: 'Ukuran ↓',    fn: (a,b) => b.fileSizeRaw   - a.fileSizeRaw   },
        ...
        this.showToast(`Diurutkan: ${mode.label}`, 'info');
    }
    ...
    async startRecording() {
        try {
            ...
            this.showToast('Merekam… ketuk Stop selesai', 'info');

        } catch (err) {
            console.error('Recording error:', err);
            this.showToast('Akses mikrofon ditolak', 'error');
        }
    }
    ...
    stopRecording() {
        if (this.mediaRecorder && this.isRecording) {
            ...
            this.showToast('Rekaman selesai — menganalisis…', 'success');
        }
    }
    ...
    setRecordBtnState(on) {
        ...
        if (!btn) return;
        btn.innerHTML = on
            ? '<i class="fa-solid fa-stop"></i> Stop Rekaman'
    ...
    clearAllResults() {
        if (!this.results.size && !this.queue.size) {
            this.showToast('Tidak ada yang dihapus', 'warning'); return;
        }
        if (!confirm('Hapus semua hasil analisis? Tindakan ini tidak bisa dibatalkan.')) return;
        ...
        this.showToast('Semua hasil dihapus', 'success');
    }
