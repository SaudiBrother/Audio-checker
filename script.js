...
class TrueAudioDetector {
    ...
    async analyzeAudio(fileData) {
        ...
        try {
            ...
            /* Close context as soon as PCM is available — avoids resource leak */
    ...
    async spectralAnalysis(audioBuffer) {
        ...
        const confidence = this.confidence(freq, cutoff);
    ...
    findCutoff(freq, sampleRate) {
        ...
        const sm      = this.smooth(freq, 5);
        const thr     = -65;          // dBFS threshold
        let   last    = 0;
        ...
        const hz          = Math.round((last / freq.length) * nyquist);
        const isArtificial = this.detectArtificial(sm, last);

        return { frequency: hz, bin: last, isArtificial };
    }

    detectArtificial(sm, cutBin) {
        const w = Math.min(40, sm.length - cutBin - 1);
        if (w < 10) return false;
        let slopeSum = 0;
        for (let i = 1; i <= w; i++) slopeSum += sm[cutBin+i] - sm[cutBin+i-1];
        return (slopeSum / w) < -2.5;
    }
    ...
    confidence(freq, cutoff) {
        let c = 100;
        if (this.dynRange(freq) < 25)  c *= 0.78;
        if (cutoff.isArtificial)        c *= 0.62;
    ...
    evaluateQuality(analysis, sampleRate) {
        ...
        if      (hz >= 20000) { tier = this.tiers.lossless; score = 100; }
        else if (hz >= 18500) { tier = this.tiers.hq;       score = 85  + ((hz - 18500) / 1500) * 15; }
        else if (hz >= 16000) { tier = this.tiers.moderate; score = 60  + ((hz - 16000) / 2500) * 25; }
        else if (hz >= 14000) { tier = this.tiers.low;      score = 30  + ((hz - 14000) / 2000) * 30; }
        else                  { tier = this.tiers.fake;     score = Math.max(5, (hz / 14000) * 30); }

        if (hfEnergy > 5)    score += 3;
        if (flatness  > 0.8) score += 3;
        if (dynRange  > 50)  score += 4;
        if (cutoff.isArtificial) score -= 15;
        ...
        return {
            ...
            normalizedFreq: Math.min(100, (hz / nyq) * 100).toFixed(1)
        };
    }
    ...
    /* ──────────────────────────────────────────────────────────
       19. SERVICE WORKER (registration now in inline bootstrap)
    ─────────────────────────────────────────────────────────── */
